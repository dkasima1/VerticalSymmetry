...
exp_name = 'Sameness - Same Pairs'  # from the Builder filename that created this script
...
filename = _thisDir + os.sep + u'data_same_pairs/' + subj_file
...
same_cond = ['same', 'different'] # categories: color, poly, chinese character, letter, object, cube
...
for trial_id, trial in enumerate(trials):
    ...
    randSame = same_list[random.randrange(len(same_list))]
    randDiff = diff_list[random.randrange(len(diff_list))]
    while randDiff[0] == randSame[0] or randDiff[1] == randSame[1]:
        randDiff = diff_list[random.randrange(len(diff_list))]
    if target_present == 'yes' and same_cond == 'same':
        for i in range(load - 1):
            twoLetters.append(randDiff)
        twoLetters.append(randSame)
    ...
    t = expClock.getTime() # get current time
